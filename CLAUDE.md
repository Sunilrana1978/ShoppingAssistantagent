...
Microservice (Firestore-backed user-memory API backing the OpenAPI toolsets, *and* the chat-proxy endpoint backing `web/index.html`, deployed separately to Cloud Run):
```bash
cd microservice && pip install -r requirements.txt && uvicorn main:app --reload
pytest microservice/test_main.py microservice/test_chat.py
...
Local test pages — the custom chat UI (primary) and the legacy SDK embed (reference), both static, no build step, no dependencies — see "Web chat UI" below:
```bash
cd web && python3 -m http.server 8000   # open http://localhost:8000 (custom UI) or /widget-embed.html (SDK embed)
```
...
**Web chat UI (`web/`) is a separate, static concern from the CXAS app itself** — neither page is pushed by `cxas push`/`build_app.py`, and `web/` itself has no Python. There are two pages:

- **`web/index.html` (primary) is a fully custom, hand-built chat UI that does not use Google's `chat-messenger` SDK.** It calls a chat-proxy endpoint, `POST /api/v1/chat/{session_id}/messages`, added to the `shopping-user-service` microservice (`microservice/main.py` + `microservice/ces_session_service.py`), which calls the CES Sessions API server-side via `cxas_scrapi.core.sessions.Sessions` (ADC credentials, same mechanism `scripts/smoke_test_routing.py` uses) and returns `{text, widget, session_ended, agent_transfer}`. This exists because the prebuilt SDK widget can't render this app's `WidgetTool`s (`show_product_carousel`/`show_product_comparison`/`show_order_summary`) — the custom UI renders them itself from the CES response's "custom payload" field. Two things learned by testing this live against the dev app rather than assuming: (1) `get_structured_response()["agent_transfer"]` is a raw `AgentTransfer` protobuf message, not a plain string — FastAPI's `jsonable_encoder` crashes on it with a 500 if returned as-is; `ces_session_service.py`'s `_agent_transfer_name()` extracts `.display_name` before returning. (2) the widget payload does carry a platform-injected `type` field derived from the tool's `widgetType`, confirmed live for the carousel — `{"type": "product_detail_carousel", "productDetails": [...]}` — even though `show_product_carousel.json`'s own params schema doesn't declare a `type` field at all, so it isn't just the model's raw tool-call args passed through verbatim. `show_product_comparison`'s/`show_order_summary`'s exact `type` strings weren't confirmed live (hit a GCP quota limit mid-verification), so `index.html`'s `renderWidget()` falls back to structural matching (`features`/`costBreakdown` presence) for those two, which is schema-guaranteed either way. Also note: the microservice's *Cloud Run* runtime service account still needs its own IAM grant to call the CES SessionService — this was verified working using this dev machine's own `gcloud` ADC credentials, a different identity than Cloud Run's default Compute Engine SA, and `microservice/deploy.sh` doesn't set that grant up yet.
- **`web/widget-embed.html` (kept for reference) is the original page**, embedding Google's prebuilt `chat-messenger` SDK (`gstatic.com/chat-messenger/sdk/prod/v1.16/`), pointed at a specific `Deployment` resource (`deploymentName`, format `projects/{project}/locations/{location}/apps/{app}/deployments/{deployment}` — note this uses the numeric GCP **project number**, not the project ID, and is a different resource from the `App` used everywhere else in this repo). Two non-obvious facts about its auth flow, found by pausing on caught exceptions in the SDK's minified source (`chat-messenger.js`) after the widget failed silently — no console error, no network request, just a swallowed exception:
